...
//!
//! Clean-room (see AGENTS.md): implemented independently from the public
//! algorithm description in RFC 1321 (<https://www.rfc-editor.org/rfc/rfc1321>),
//! reproducing PG's observable `md5()` output — the 32-character lowercase hex
//! digest — as pinned by the `md5` regression test. No PG C source is consulted.
...
pub fn md5_hex(data: &[u8]) -> String {
    let [a0, b0, c0, d0] = md5(data);
    let mut out = Vec::with_capacity(32);
    // The digest is the four state words in little-endian byte order.
...
/// The four MD5 state words after processing `data`.
fn md5(data: &[u8]) -> [u32; 4] {
    ...
    // so the total is a multiple of 64 bytes (RFC 1321 §3.1-3.2).
    ...
    for chunk in msg.chunks_exact(64) {
        // Decode the 512-bit chunk into 16 little-endian 32-bit words.
        let mut m = [0u32; 16];
