...
use crabgresql_storage_api::{
    IndexConstraint, IndexMetadata, PartitionStrategy, RelPersistence, RelStats, RelationFilenodes,
...
impl SystemCatalog {
    ...
    ///
    /// `None` covers both misses PostgreSQL reports as NULL: an OID no relation
    /// answers to, and a relation that is not partitioned. Column names rather
    /// than positions, for the reason [`Self::constraint_def`] gives — the
    /// caller renders DDL and holds no schema.
