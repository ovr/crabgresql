...
-- NULL arguments do not participate; every argument NULL yields NULL (renders
-- empty), and an all-untyped list resolves to text as it does for COALESCE
...
-- every argument is evaluated, unlike COALESCE's: nothing is skipped because an
-- earlier one already decided the answer
SELECT GREATEST(1, 1/0);
-- the ordering is the type's own. float8 puts NaN above every number, and bool
-- has false < true
...
-- a string compares under the collation the arguments derive, so the answer
-- moves with it: byte order (C) puts 'a' (0x61) above 'B' (0x42), a linguistic
-- one does not
...
-- timestamps, dates and intervals order naturally
SELECT GREATEST('2024-03-01'::date, '2024-01-01'::date) AS later,
       LEAST('1 day'::interval, '2 hours'::interval) AS shorter;
-- over a table column, once per row; in a WHERE predicate, under an aggregate,
-- and as a GROUP BY key
...
DROP TABLE defaults;
-- a view keeps the same spelling. Known gap: on this path the literal arguments
-- are not type-labelled (`NULL` rather than `NULL::integer`), which COALESCE
-- shares. A type modifier survives only when every argument agrees on it -- read
-- off a column reference, since a *folded* constant loses the cast that carried
-- it, another gap COALESCE shares
CREATE VIEW v AS
