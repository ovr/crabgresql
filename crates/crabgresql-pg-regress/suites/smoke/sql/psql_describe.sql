...
\d dfmt_def
DROP TABLE dfmt_def;
-- An array column's Collation stays empty. `\d` fills that cell only when the
-- column's attcollation differs from its type's typcollation, so this is the
-- test that the two agree: `text[]` carries `default` and `name[]` carries `C`
-- in both pg_type and pg_attribute, exactly as their elements do.
CREATE TABLE arrcoll (t text[], n name[], i integer[]);
\d arrcoll
SELECT a.attname, a.attcollation = t.typcollation AS agrees
  FROM pg_catalog.pg_attribute a
       JOIN pg_catalog.pg_type t ON t.oid = a.atttypid
 WHERE a.attrelid = 'arrcoll'::regclass AND a.attnum > 0
 ORDER BY a.attnum;
DROP TABLE arrcoll;
