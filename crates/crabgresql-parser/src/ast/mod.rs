...
pub enum CreateFunctionBody {
    ...
    /// The SQL-standard body of a `LANGUAGE SQL` routine: a block of statements
    /// bound to the routine at definition time rather than a quoted string
    /// parsed on first call.
    ///
    /// Example:
    /// ```sql
