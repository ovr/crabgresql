...
/// PostgreSQL's fully reserved keywords: the ones that cannot be a **column
/// name** unless quoted.
///
...
///
/// ```sql
/// SELECT upper(word) FROM pg_get_keywords() WHERE catcode IN ('R','T') ORDER BY word;
/// ```
///
/// `R` is `reserved` and `T` is `type_func_name_keyword`; PG rejects both in a
/// column name. The other two categories — `U` (unreserved) and `C`
