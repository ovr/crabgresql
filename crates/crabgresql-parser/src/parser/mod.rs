...
impl<'a> Parser<'a> {
    ...
    /// Consume `BY DEFAULT AS` when it is followed by `(`, returning the span of
    /// the `BY` token — PostgreSQL's cursor for this clause sits there, not on
    /// the `GENERATED` the caller already consumed.
    ///
    ...
    fn parse_by_default_as_expr_start(&mut self) -> Option<Span> {
        let start = self.index;
        let keyword = self.peek_token_ref().span;
    ...
    fn parse_optional_column_option_generated(
        ...
        } else if let Some(keyword) = self.parse_by_default_as_expr_start() {
