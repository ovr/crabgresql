...
impl<'a> Parser<'a> {
    ...
    fn parse_expr_prefix_by_reserved_word(
        ...
        match w.keyword {
            ...
            Keyword::ARRAY if self.peek_token_ref().token == Token::LParen => {
                self.expect_token(&Token::LParen)?;
            ...
            Keyword::MAP
                if *self.peek_token_ref() == Token::LBrace
                        ...
                        match exprs.len() {
                            ...
                            1 => {
                                let Some(expr) = exprs.into_iter().next() else {
                                    return Err(ParserError::ParserError(
                                        "Internal parser error: single expression is missing"
                                            .to_string(),
                                    ));
                                };
        ...
        let params = if self.consume_token(&Token::LParen) {
            ...
            if params.len() == 1 {
                let Some(param) = params.into_iter().next() else {
                    return Err(ParserError::ParserError(
                        "Internal parser error: lambda parameter is missing".to_string(),
                    ));
                };
                OneOrManyWithParens::One(param)
            } else {
                OneOrManyWithParens::Many(params)
            }
        };
    ...
    fn parse_function_call(&mut self, name: ObjectName) -> Result<Function, ParserError> {
        ...
        if dialect_of!(self is GenericDialect) && self.consume_token(&Token::LParen) {
    ...
    /// Parse the frame-exclusion clause that follows `EXCLUDE`.
    ...
    pub fn parse_infix(&mut self, expr: Expr, precedence: u8) -> Result<Expr, ParserError> {
        ...
        let regular_binary_operator = match &tok.token {
            ...
            Token::DuckIntDiv if dialect_is!(dialect is GenericDialect) => {
    ...
    /// let dialect = GenericDialect {};
    /// let mut parser = Parser::new(&dialect).try_with_sql("ORDER BY foo, bar")?;
    ///
    ...
    fn parse_create_cast(&mut self) -> Result<Statement, ParserError> {
        ...
        let context = if self.parse_keyword(Keyword::AS) {
            ...
            match word.value.to_ascii_lowercase().as_str() {
                ...
                _ => {
                    return self
                        .expected_ref("ASSIGNMENT or IMPLICIT after AS", self.peek_token_ref());
                }
            }
        } else {
    ...
    pub fn parse_create_role(&mut self) -> Result<CreateRole, ParserError> {
        ...
        let optional_keywords = if dialect_of!(self is PostgreSqlDialect) {
            vec![
                Keyword::LOGIN,
                Keyword::NOLOGIN,
                Keyword::INHERIT,
    ...
    fn parse_optional_column_option_inner(&mut self) -> Result<Option<ColumnOption>, ParserError> {
        ...
        } else if self.parse_keyword(Keyword::AUTO_INCREMENT) && dialect_of!(self is GenericDialect)
        {
            ...
            && dialect_of!(self is GenericDialect)
        {
            ...
            // The clause's own start: PostgreSQL points its cursor at the
            // `GENERATED` keyword when it refuses one.
            let keyword = self.get_current_token().span;
            self.parse_optional_column_option_generated(keyword)
        ...
        } else if self.parse_keyword(Keyword::AS) && dialect_of!(self is GenericDialect) {
            self.parse_optional_column_option_as()
        } else if self.parse_keyword(Keyword::SRID) && dialect_of!(self is GenericDialect) {
            Ok(Some(ColumnOption::Srid(Box::new(self.parse_expr()?))))
        } else if self.parse_keyword(Keyword::IDENTITY) && dialect_of!(self is GenericDialect) {
    ...
    /// Whether what follows is `BY DEFAULT AS (`, consuming the keywords when
    /// it is. `keyword` is set to the `BY` token's span: PostgreSQL's cursor for
    /// this clause sits there, not on the `GENERATED` already consumed.
    ///
    /// A miss rewinds — the way [`Parser::try_parse`] does — so that the three
    /// keywords are not eaten on the way out. Without that, a malformed
    /// `GENERATED BY DEFAULT AS foo` resumed parsing mid-clause and complained
    /// about the wrong token.
    fn peek_by_default_as_expr(&mut self, keyword: &mut Span) -> bool {
        let start = self.index;
        *keyword = self.peek_token_ref().span;
        if self.parse_keywords(&[Keyword::BY, Keyword::DEFAULT, Keyword::AS])
    ...
    fn parse_optional_column_option_generated(
        ...
        if self.parse_keywords(&[Keyword::ALWAYS, Keyword::AS, Keyword::IDENTITY]) {
            ...
            if self.expect_token(&Token::LParen).is_ok() {
                let expr: Expr = self.with_state(ParserState::Normal, |p| p.parse_expr())?;
                self.expect_token(&Token::RParen)?;
                // `STORED` and `VIRTUAL` are both PostgreSQL 18 spellings, and
                // a clause naming neither is `VIRTUAL` there — the caller
                // decides what an absent modifier means for its dialect.
        ...
        } else if self.peek_by_default_as_expr(&mut keyword) {
            // `GENERATED BY DEFAULT AS (expr)`: parsed rather than rejected as
            // syntax, so the caller can give it PostgreSQL's own complaint —
            // that a generated column must say GENERATED ALWAYS.
            self.expect_token(&Token::LParen)?;
            let expr: Expr = self.with_state(ParserState::Normal, |p| p.parse_expr())?;
            self.expect_token(&Token::RParen)?;
            let expr_mode = if self.parse_keywords(&[Keyword::STORED]) {
                Some(GeneratedExpressionMode::Stored)
            } else if self.parse_keywords(&[Keyword::VIRTUAL]) {
    ...
    fn parse_optional_column_option_as(&mut self) -> Result<Option<ColumnOption>, ParserError> {
        // Some DBs allow 'AS (expr)', shorthand for GENERATED ALWAYS AS. The
        // `AS` stands in for the `GENERATED` keyword as the clause's start.
        let keyword = self.get_current_token().span;
        self.expect_token(&Token::LParen)?;
        let expr = self.parse_expr()?;
        self.expect_token(&Token::RParen)?;
    ...
    pub fn parse_optional_table_constraint(
        ...
        match next_token.token {
            Token::Word(w) if w.keyword == Keyword::UNIQUE => {
                ...
                if !dialect_of!(self is GenericDialect) && !index_type_display.is_none() {
                    return self.expected_ref(
    ...
    pub fn parse_sql_option(&mut self) -> Result<SqlOption, ParserError> {
        ...
        match &self.peek_token_ref().token {
            Token::Word(w) if w.keyword == Keyword::HEAP && is_generic => {
                Ok(SqlOption::Ident(self.parse_identifier()?))
            }
            Token::Word(w) if w.keyword == Keyword::PARTITION && is_generic => {
                self.parse_option_partition()
            }
            Token::Word(w) if w.keyword == Keyword::CLUSTERED && is_generic => {
    ...
    fn parse_literal_char(&mut self) -> Result<char, ParserError> {
        ...
        match s.chars().next() {
            Some(ch) => Ok(ch),
            None => Err(ParserError::ParserError("Expected a character".to_string())),
        }
    }
    ...
    /// Parse a routine body written as a literal — the `$$ ... $$` or `'...'`
    /// after `CREATE FUNCTION ... AS`, `CREATE PROCEDURE ... AS`, or `DO`.
    ///
    /// The span is kept because its `start` is the position of the opening
    /// delimiter, which is what lets an error raised inside the body be mapped
    /// back to a line of the statement that declared it.
    fn parse_routine_body_literal(&mut self) -> Result<ValueWithSpan, ParserError> {
        ...
        match peek_token.token {
            ...
            _ => Ok(Value::SingleQuotedString(self.parse_literal_string()?).with_span(span)),
        }
    }
    ...
    fn parse_create_function_body_string(&mut self) -> Result<CreateFunctionBody, ParserError> {
        Ok(CreateFunctionBody::AsBeforeOptions {
            body: Expr::Value(self.parse_routine_body_literal()?),
            link_symbol: if self.consume_token(&Token::Comma) {
                Some(Expr::Value(self.parse_routine_body_literal()?))
            } else {
                None
            },
        })
    ...
    /// Parse `DO [ LANGUAGE lang_name ] code` — an anonymous code block,
    /// executed as if it were the body of a void-returning procedure.
    /// PostgreSQL accepts the LANGUAGE clause on either side of the code.
    ///
    ...
    fn parse_do(&mut self) -> Result<Statement, ParserError> {
        ...
        let body = self.parse_routine_body_literal()?;
        ...
        Ok(Statement::Do(DoStatement { language, body }))
    }
    ...
    /// `foo` or `myschema."table"
    ///
    /// The `_in_table_clause` parameter is retained for signature compatibility with
    /// [`Parser::parse_object_name`] but is no longer used (it previously toggled BigQuery
    /// unquoted-hyphenated-identifier parsing).
    ///
    ...
    /// let actual = parser.parse_multipart_identifier()?;
    /// assert_eq!(&actual, &expected);
    ///
    /// // parse_identifiers is more loose on what it allows, parsing successfully
    /// let sql = "one + two";
    /// let mut parser = Parser::new(&dialect).try_with_sql(sql)?;
    /// let actual = parser.parse_identifiers()?;
    /// assert_eq!(&actual, &expected);
    ///
    /// // expected to strictly fail due to + separator
    /// let sql = "one + two";
    /// let mut parser = Parser::new(&dialect).try_with_sql(sql)?;
    /// let actual = match parser.parse_multipart_identifier() {
    ///     Err(error) => error,
    ///     Ok(_) => panic!("expected invalid multipart identifier"),
    /// };
    /// assert_eq!(
    ///     actual.to_string(),
    ...
    pub fn parse_query_body(&mut self, precedence: u8) -> Result<Box<SetExpr>, ParserError> {
            ...
            || (self.peek_keyword(Keyword::FROM) && self.dialect.supports_from_first_select())
        {
            ...
            SetExpr::Values(self.parse_values(false, false)?)
        } else if self.parse_keyword(Keyword::VALUE) {
            SetExpr::Values(self.parse_values(false, true)?)
        } else if self.parse_keyword(Keyword::TABLE) {
    ...
    fn parse_remaining_set_exprs(
        ...
        loop {
            ...
            let set_quantifier = self.parse_set_quantifier(&Some(op));
    ...
    pub fn parse_show(&mut self) -> Result<Statement, ParserError> {
        // PostgreSQL only supports `SHOW <name>` / `SHOW ALL`.
    ...
    pub fn parse_table_factor(&mut self) -> Result<TableFactor, ParserError> {
            ...
            && self.parse_keyword(Keyword::UNNEST)
        {
            ...
            let partitions: Vec<Ident> =
                if dialect_of!(self is GenericDialect) && self.parse_keyword(Keyword::PARTITION) {
                    self.parse_parenthesized_identifiers()?
                } else {
                    vec![]
                };
    ...
    pub fn parse_grantee_name(&mut self) -> Result<GranteeName, ParserError> {
        ...
        {
            let Some(part) = name.0.pop() else {
                return Err(ParserError::ParserError(
                    "Expected a grantee name".to_string(),
                ));
            };
            let Some(user) = part.as_ident().cloned() else {
                return Err(ParserError::ParserError(
                    "Expected a grantee identifier".to_string(),
                ));
            };
    ...
    pub fn parse_insert(&mut self, insert_token: TokenWithSpan) -> Result<Statement, ParserError> {
        ...
        let priority = if !dialect_of!(self is GenericDialect) {
            None
        ...
        let ignore = dialect_of!(self is GenericDialect) && self.parse_keyword(Keyword::IGNORE);
            ...
            .parse_keywords(&[Keyword::DEFAULT, Keyword::VALUES])
        {
            ...
            let (columns, partitioned, after_columns) = if !self.peek_subquery_start() {
                let columns = self.parse_parenthesized_qualified_column_list(Optional, false)?;
        ...
        let insert_alias = if dialect_of!(self is GenericDialect) && self.parse_keyword(Keyword::AS)
        {
    ...
    fn parse_function_argument_list(&mut self) -> Result<FunctionArgumentList, ParserError> {
        ...
        if dialect_of!(self is GenericDialect) && self.parse_keyword(Keyword::SEPARATOR) {
    ...
    pub fn parse_load(&mut self) -> Result<Statement, ParserError> {
        if self.dialect.supports_load_extension() {
            ...
            self.expected_ref("an extension name after `LOAD`", self.peek_token_ref())
        }
    }
...
mod tests {
    ...
    mod test_parse_data_type {
            ...
            ($dialect:expr, $input:expr, $expected_type:expr $(,)?) => {{
                $dialect.run_parser_method(&*$input, |parser| {
                    let data_type = match parser.parse_data_type() {
                        ...
                        Err(error) => panic!("failed to parse data-type test fixture: {error}"),
                    };
        ...
        #[test]
        fn test_ansii_exact_numeric_types() -> anyhow::Result<()> {
            ...
            dialect.run_parser_method(
                "NUMERIC(10,+5)",
                |parser| -> Result<(), crate::parser::ParserError> {
    ...
    #[test]
    fn test_parse_schema_name() -> anyhow::Result<()> {
        ...
        macro_rules! test_parse_schema_name {
            ($input:expr, $expected_name:expr $(,)?) => {{
                all_dialects().run_parser_method(&*$input, |parser| -> Result<(), ParserError> {
                    let schema_name = parser.parse_schema_name()?;
    ...
    #[test]
    fn mysql_parse_index_table_constraint() -> anyhow::Result<()> {
        macro_rules! test_parse_table_constraint {
            ($dialect:expr, $input:expr, $expected:expr $(,)?) => {{
                $dialect.run_parser_method(&*$input, |parser| -> Result<(), ParserError> {
                    let constraint =
    ...
    #[test]
    fn test_parse_multipart_identifier_positive() -> anyhow::Result<()> {
        let dialect = TestedDialects::new(vec![Box::new(GenericDialect {})]);
        ...
        dialect.run_parser_method(
            r#"CATALOG."F(o)o. ""bar".table"#,
            |parser| -> Result<(), ParserError> {
                let actual = parser.parse_multipart_identifier()?;
        ...
        dialect.run_parser_method("CATALOG . table", |parser| -> Result<(), ParserError> {
            let actual = parser.parse_multipart_identifier()?;
            assert_eq!(expected, actual);
            Ok(())
        })?;
    ...
    #[test]
    fn test_mysql_partition_selection() -> anyhow::Result<()> {
        let sql = "SELECT * FROM employees PARTITION (p0, p2)";
        let expected = vec!["p0", "p2"];

        let ast: Vec<Statement> = Parser::parse_sql(&GenericDialect {}, sql)?;
        assert_eq!(ast.len(), 1);
    ...
    fn parse_pg(sql: &str) -> Result<Vec<Statement>, ParserError> {
        Parser::parse_sql(&PostgreSqlDialect {}, sql)
    }

    fn do_stmt(sql: &str) -> DoStatement {
        match parse_pg(sql).expect("parse").pop() {
            Some(Statement::Do(stmt)) => stmt,
            other => panic!("expected DO, got {other:?}"),
        }
    }

    /// The body of a `DO` block is captured verbatim, whichever quoting form it
    /// uses, and the LANGUAGE clause is accepted on either side of it.
    #[test]
    fn parse_do_captures_body_verbatim() {
        let untagged = do_stmt("DO $$ BEGIN RAISE NOTICE 'hi'; END $$");
        assert!(untagged.language.is_none());
        assert_eq!(
            untagged.body.value,
        ...
        // A tag lets the body contain `$$`, and nested tags stay verbatim.
        let tagged = do_stmt("DO $outer$ SELECT $$inner$$; $outer$");
        assert_eq!(
            tagged.body.value,
            Value::DollarQuotedString(DollarQuotedString {
                value: " SELECT $$inner$$; ".to_string(),
                tag: Some("outer".to_string()),
            })
        );

        // `DO 'code'` — the SQL-standard single-quoted spelling.
        let quoted = do_stmt("DO 'BEGIN END'");
        assert_eq!(
            quoted.body.value,
    ...
    /// The body literal's span starts at the opening delimiter, which is what
    /// lets a position inside the body map back to a line of the statement.
    #[test]
    fn parse_do_body_span_points_at_the_opening_delimiter() {
        let stmt = do_stmt("DO $tag$\nBEGIN\nEND\n$tag$");
        assert_eq!(stmt.body.span.start.line, 1);
        // 1-based column of the `$` that opens `$tag$`.
        assert_eq!(stmt.body.span.start.column, 4);
    }

    /// PostgreSQL rejects a second LANGUAGE clause with "conflicting or
    ...
    /// `DO` is only a statement in leading position; `ON CONFLICT DO NOTHING`
    /// and friends consume their own `DO` and must not be disturbed.
    #[test]
    fn do_keyword_elsewhere_is_untouched() {
        parse_pg("INSERT INTO t VALUES (1) ON CONFLICT DO NOTHING").expect("parse");
    }

    /// `DO` round-trips through `Display`, always printing LANGUAGE first.
    #[test]
    fn parse_do_round_trips() {
