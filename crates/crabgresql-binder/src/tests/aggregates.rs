...
fn aggregate_order_by_becomes_sort_keys_on_the_aggregate() -> anyhow::Result<()> {
    ...
    // The key indexes the *source* row, exactly as the argument does — the
    // marker is what becomes a ColumnRef into the aggregate's output.
...
fn distinct_aggregate_order_by_must_name_an_argument() -> anyhow::Result<()> {
    ...
    // The spelling where they do coincide binds.
    assert_eq!(
