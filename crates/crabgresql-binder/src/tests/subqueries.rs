...
#[test]
fn not_exists_sets_negated() -> anyhow::Result<()> {
    let QueryPlan { predicate, .. } =
...
#[test]
fn not_in_subquery_binds_to_all_of_inequality() -> anyhow::Result<()> {
    let QueryPlan { predicate, .. } =
...
#[test]
fn scalar_subquery_multiple_columns_errors() -> anyhow::Result<()> {
...
#[test]
fn in_subquery_multiple_columns_errors() -> anyhow::Result<()> {
...
#[test]
fn correlated_qualified_reference_binds_to_outer_column() -> anyhow::Result<()> {
    ...
    let pred = bound_query("SELECT id FROM t x WHERE EXISTS (SELECT 1 FROM t WHERE id = x.id)")?
        .predicate
        .context("a WHERE predicate")?;
    let BoundExpr::Exists { subplan, negated } = pred else {
        bail!("expected EXISTS marker, got {pred:?}");
    };
    assert!(!negated);
    ...
    let BoundExpr::Binary { right, .. } = inner else {
        bail!("expected `id = x.id` comparison, got {inner:?}");
    };
    assert!(
        matches!(
            **right,
...
#[test]
fn correlated_unqualified_reference_binds_to_outer_column() -> anyhow::Result<()> {
...
#[test]
fn uncorrelated_missing_column_still_errors_42703() -> anyhow::Result<()> {
...
#[test]
fn scalar_subquery_column_named_after_inner_column() -> anyhow::Result<()> {
...
#[test]
fn exists_column_named_exists() -> anyhow::Result<()> {
...
#[test]
fn update_set_accepts_subquery() -> anyhow::Result<()> {
...
#[test]
fn delete_where_accepts_in_subquery() -> anyhow::Result<()> {
    let DeletePlan { predicate, .. } =
        bound_delete("DELETE FROM t WHERE id IN (SELECT id FROM t)")?;
    assert!(matches!(
        predicate,
