...
fn scalar_subquery_binds_with_column_type() -> anyhow::Result<()> {
    ...
    let LogicalPlan::Values(ValuesPlan { rows, .. }) = bound("SELECT (SELECT big FROM t)") else {
...
fn scalar_subquery_column_named_after_inner_column() {
    let LogicalPlan::Values(ValuesPlan { columns, .. }) = bound("SELECT (SELECT max(id) FROM t)")
    else {
        panic!("expected Values");
    };
...
fn exists_column_named_exists() {
    let LogicalPlan::Values(ValuesPlan { columns, .. }) = bound("SELECT EXISTS (SELECT 1 FROM t)")
    else {
        panic!("expected Values");
    };
