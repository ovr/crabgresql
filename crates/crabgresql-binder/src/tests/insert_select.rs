...
fn insert_select_binds_as_query_source() -> anyhow::Result<()> {
    ...
    let LogicalPlan::Insert(InsertPlan {
        ...
        ..
    }) = bound("INSERT INTO t (id, name) SELECT id, name FROM t")?
    else {
...
#[test]
fn insert_select_arity_mismatches_match_pg() -> anyhow::Result<()> {
...
#[test]
fn insert_select_type_mismatch_reports_datatype_mismatch() -> anyhow::Result<()> {
...
fn table_preserves_quoted_identifier_case() -> anyhow::Result<()> {
    ...
    match bind("TABLE MixedCase") {
        Err(e) => assert_eq!(e.downcast::<BindError>()?.code, sqlstate::UNDEFINED_TABLE),
        Ok(_) => bail!("unquoted MixedCase must not resolve to \"MixedCase\""),
    }
    Ok(())
}
...
fn insert_source_query_clauses_are_executed_not_rejected() -> anyhow::Result<()> {
    ...
    for sql in [
        ...
        let LogicalPlan::Insert(InsertPlan {
            ...
            bail!("expected a query-source Insert for: {sql}");
        };
    }
    Ok(())
}
...
fn default_keyword_binds_as_typed_null_without_a_declared_default() -> anyhow::Result<()> {
    let LogicalPlan::Insert(InsertPlan {
        ...
        ..
    }) = bound("INSERT INTO t (id) VALUES (DEFAULT)")?
    else {
        bail!("expected Insert with a VALUES source");
    };
    assert_eq!(
        rows[0][0],
        BoundExpr::Const {
    ...
    assert!(bound("UPDATE t SET id = DEFAULT").is_ok());
...
fn returning_binds_output_columns_for_each_dml() -> anyhow::Result<()> {
    ...
    let LogicalPlan::Insert(InsertPlan {
        ...
        bail!("expected Insert with RETURNING");
    };
    ...
    assert_eq!(
        cols.last()
            .context("RETURNING produced at least one column")?
            .ty,
        PgType::Int4
    );
...
#[test]
fn returning_rejects_aggregates_and_set_returning_functions() -> anyhow::Result<()> {
...
#[test]
fn ragged_values_lists_are_42601() -> anyhow::Result<()> {
