...
pub enum BoundExpr {
    ...
    Aggregate {
        ...
        ///
        /// Deliberately *not* named `args`: this variant is the one whose
        /// arguments are not all the row expressions it holds (`order_by` are
        /// too), and the name keeps it out of the `FuncCall { args, .. } | …`
        /// or-patterns below, where joining it would silently skip those keys.
        /// Traversals go through [`BoundExpr::agg_exprs`].
...
impl BoundAggregate {
    ...
    /// arguments first. The order matches [`Self::exprs_mut`], so the two can be
    /// zipped — the same pairing [`BoundWindowSpec::exprs`] makes, and for the
    /// same reason: a caller that rebases column indexes or resolves subqueries
    /// must reach the `ORDER BY` keys too, not only `args`.
...
impl BoundExpr {
    ...
    pub fn column_ref_bounds(&self) -> Option<(usize, usize)> {
        fn fold(expr: &BoundExpr, acc: &mut Option<(usize, usize)>) {
            match expr {
                ...
                // The arguments and the aggregate's own ORDER BY both read this
                // row, so both widen the hull — the same rule the window arm
                // below applies to its OVER clause.
                BoundExpr::Aggregate {
                    agg_args, order_by, ..
                } => {
    ...
    /// row it sits in — its arguments, then its own `ORDER BY` keys.
    ///
    /// The marker's traversals go through this rather than reading `agg_args`
    /// alone: an `ORDER BY` key holds `ColumnRef`s into the same row, so a walk
    /// that skipped it would leave them behind when the row is relocated. It is
    /// the counterpart of [`BoundWindowSpec::exprs`], which exists for the same
    /// reason on the window marker.
    fn agg_exprs<'a>(
        args: &'a [BoundExpr],
    ...
    pub fn shift_column_refs(&mut self, delta: isize) {
        match self {
            ...
            // Mirrors `column_ref_bounds`: the arguments and the aggregate's own
            // ORDER BY both index this row, so both move with it.
            BoundExpr::Aggregate {
                agg_args, order_by, ..
            } => {
