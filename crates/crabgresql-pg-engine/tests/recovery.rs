...
fn open(dir: &Path) -> (PgEngine, TransactionManager) {
...
#[test]
fn committed_survives_uncommitted_and_aborted_vanish() {
    ...
    // --- lifetime 2: recover and verify. ---
    {
        ...
        assert!(next > xid_high, "next XID {next:?} must exceed recovered {xid_high:?}");
...
fn replaying_the_same_wal_twice_is_idempotent() {
    ...
    let engine2 = PgEngine::new(dir.path(), Arc::clone(&wal), &mut reg).unwrap();
    let clog = Arc::new(Clog::new());
