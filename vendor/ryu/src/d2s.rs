...
pub fn d2d(ieee_mantissa: u64, ieee_exponent: u32) -> FloatingDecimal64 {
    ...
    // PostgreSQL modification: force accept_bounds = false (STRICTLY_SHORTEST
    // disabled) so output never lands on a float-boundary midpoint, matching
    // PG float8out byte-for-byte.
    let even = (m2 & 1) == 0;
    let _ = even;
