...
runs before `cargo test --workspace` — covers it. It stays a manual step because
cargo has no stable way for a crate to depend on another crate's *binary*, and a
nested `cargo build` inside a test would block on the outer build-directory
lock.
